...
public class ExecutionTutorial : MonoBehaviour
{
    ...
    public void hideGoals() {
        StartCoroutine(hideGoalsIE(1f));
    }

    private IEnumerator hideGoalsIE(float time) {
        yield return new WaitForSeconds(time);
        ToggleBoolAnimator(goals);
        yield break;
    }
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(ray, out hitObject))
        {
            if (Input.GetMouseButtonDown(0))
            {
                ...
                {
                    EventManager.TriggerEvent("clicked"+ hitObject.collider.GetComponent<BuildingType>().type.ToString());
